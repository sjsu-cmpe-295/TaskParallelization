...
public class HeartbeatSenderTask extends TimerTask {
	...
	@Override
	public void run() {
		Cluster cluster = leader.getCluster();
		if (!cluster.getPiNodes()
			.containsKey(nodeState.getRoutingConfig().getNodeId()))
			updateUI = true;
		else if (!cluster.getPiNodes()
			.get(nodeState.getRoutingConfig().getNodeId()).getPiNodeType()
			.equals(PiNodeType.MASTER))
			updateUI = true;

		masterResponse = "{\"id\": \""
			+ nodeState.getRoutingConfig().getNodeId() + "\", \"ip\": \""
			+ nodeState.getRoutingConfig().getHost() + "\",\"isMaster\": \""
			+ PiNodeType.MASTER + "\",\"state\": \"" + PiNodeState.ACTIVE
			+ "\"},";
			...
			.forEach(ri -> {
				...
				if (!updateUI) {
					if (!cluster.getPiNodes().containsKey(ri.getId()))
						updateUI = true;
					else if (!cluster.getPiNodes().get(ri.getId())
						.getPiNodeType().equals(PiNodeType.WORKER))
						// ||
				...
				piNode.setCommandPort(ri.getCommandPort());
				...
				if (cluster.getPiNodes().containsKey(ri.getId())) {
					if (!cluster.getPiNodes().get(ri.getId()).getPiNodeState()
						.equals(piNode.getPiNodeState()))
						updateUI = true;
				}
				leader.addToCluster(piNode);
				workerResponse.append("{\"id\": \"" + id + "\", \"ip\": \""
					+ ri.getHost() + "\",\"isMaster\": \"" + PiNodeType.WORKER
					+ "\",\"state\": \"" + piNode.getPiNodeState() + "\"},");
	...
	public void updateUI() {
		...
		try {
			URL url = new URL(
				"http://" + nodeState.getRoutingConfig().getClientIP()
					+ ":1300/updateCluster");
			...
			response.append(
				workerResponse.substring(0, workerResponse.length() - 1));
